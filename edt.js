...
async function enregistrerGrille() {
  ...
  // On efface les cases de cette semaine, puis on réécrit
  await sb
    .from("edt_cases")
    .delete()
    .eq("annee_id", anneeId)
  ...
  if (lignes.length) {
    const { error } = await sb.from("edt_cases").insert(lignes);
    if (error) throw new Error(error.message);
  }
  modifie = false;
}
...
export function renderEDT() {
  ...
  // Cas : pas encore de semaines → bouton pour les générer
  if (!semaines.length) {
    return `
      <h1>Emploi du temps</h1>
      <p class="hint">Les semaines de l'année ne sont pas encore créées.</p>
      <div class="carte">
        <button class="bouton" id="genererSemaines">Préparer l'année (créer les semaines)</button>
        <div id="messageSemaines" class="status"></div>
      </div>
    `;
  }
  ...
  const pastilles = classes
    .map((c) => {
      const actif = classeChoisie && classeChoisie.id === c.id ? " actif" : "";
      return `<button class="pastille${actif}" data-id="${c.id}" data-nom="${c.nom}"
                 style="background:${couleurClasse(c.nom)}">${c.nom}</button>`;
    })
    .join("");
  const gommeActive = classeChoisie && classeChoisie.id === "gomme" ? " actif" : "";
...
export function bindEDT() {
  ...
  if (btnGen) {
    btnGen.addEventListener("click", async () => {
      ...
      msg.textContent = "⏳ Création des semaines…";
      try {
        // Récupérer le libellé de l'année active
        const { data: annee } = await sb
          .from("annees")
          .select("libelle")
          .eq("id", anneeId)
          .maybeSingle();
        const nb = await genererSemaines(annee.libelle);
        await chargerTout();
        rafraichir();
      } catch (e) {
        msg.textContent = "❌ " + e.message;
  ...
  document.querySelectorAll(".case-edt").forEach((td) => {
    td.addEventListener("click", () => {
      ...
      const cle = `${td.dataset.jour}|${td.dataset.creneau}`;
      if (classeChoisie.id === "gomme") grille.delete(cle);
      else grille.set(cle, { id: classeChoisie.id, nom: classeChoisie.nom });
      modifie = true;
      rafraichir();
    });
  });
